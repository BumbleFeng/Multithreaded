...
public class MazeDisplay extends JPanel {
	...
	private void drawMazeCell(Position pos, int x, int y, int width, int height, Graphics g) {
		...
		if (cell != null) {
			...
			if (maze.getColor(pos) == 2 | maze.getColor(pos) == 1) {
	...
	private void drawMaze(Graphics g) {
		/*
		 * Find out how large we can make the cells. Reserve one pixel for the wall
		 * around the border. Reserve one row above and below for the entrance and exit.
		 */
		cellTypes.clear();
		numFound = 0;
		numDrawn = 0;
		...
		for (int draw_x = 2; curr.col < maze.getWidth(); curr = curr.move(Direction.EAST), draw_x += cellWidth) {
			g.setColor(lookupColor(0));
			g.drawLine(draw_x, cellHeight, draw_x + cellWidth - 2, cellHeight);
		}
		curr = new Position(0, 0);
		for (int draw_y = cellHeight + 1; curr.row < maze.getHeight(); curr = curr.move(Direction.SOUTH), draw_y += cellHeight) { 
			g.setColor(lookupColor(0));
			g.drawLine(1, draw_y, 1, draw_y + cellHeight - 2);
		}
		
		// Draw entrance and exit
		curr = new Position(maze.getWidth() / 2, 0);
		g.setColor(lookupColor(maze.getColor(curr)));
		g.fillRect(maze.getWidth() / 2 * cellWidth + 2, 0, cellWidth - 1, cellHeight + 1);
		curr = new Position(maze.getWidth() / 2, maze.getHeight() - 1);
		g.setColor(lookupColor(maze.getColor(curr)));
		g.fillRect(maze.getWidth() / 2 * cellWidth + 2, (maze.getHeight() + 1) * cellHeight, cellWidth - 1,
				cellHeight + 1);
