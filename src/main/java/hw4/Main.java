...
public class Main {
	...
	public void solve() {
		...
		MazeSolver[] solvers = {
				// new StudentSTMazeSolverRec(maze),
				// new STMazeSolverRec(maze),
				// new STMazeSolverDFS(maze),
				// new STMazeSolverBFS(maze),
				new StudentMTMazeSolverDFS(maze), // uncomment this line when you are ready to test yours
				new StudentMTMazeSolverBFS(maze), };
		...
		for (MazeSolver solver : solvers) {
			...
			if (soln == null) {
				if (!solvable) {
					...
					System.err.println("Incorrectly returned no solution when there is one.");
				}
			} else {
				if (maze.checkSolution(soln)) {
					System.out.println("Correct solution found in " + sec + " seconds.");
				} else {
					System.err.println("Incorrect solution found.");
				}
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		String mazeLocationNotInProjectFolder = "mazes/"; // replace this with your maze directory
		String maze[] = { "6x6.mz", "200x100.mz", "200x200.mz", "200x200u.mz", "500x500.mz", "1000x1000.mz",
				"4000x4000u.mz", "5000x5000.mz", "10000x10000u.mz" };
		String whichMazeToUse = maze[2]; // which maze file to load
